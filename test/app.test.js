...
test('adult account can analyze homework, gaming, and sports safely',async()=>{
  ...
  const speech=await fetch(base+'/api/speech',{method:'POST',headers:{'Content-Type':'application/json',Cookie:cookie},body:JSON.stringify({reportId:gaming.data.report.id,coach:'Byte',text:'Practice the next decision.'})});assert.equal(speech.status,200);assert.match(speech.headers.get('content-type'),/audio\/mpeg/);const health=await call('/api/health');assert.equal(health.data.elevenLabsConfigured,true);assert.ok((await speech.arrayBuffer()).byteLength>0);
